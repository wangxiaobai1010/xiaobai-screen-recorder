...
internal sealed class RecordingNativeMovingEventArgs(
...
/// content is the second live Avalonia Panel 4 view.
/// </summary>
internal sealed class RecordingFloatingForm : Form
{
    ...
    private readonly Dictionary<int, RecordingResizeGrip> _resizeGrips = [];
    private bool _nativeMoveActive;
    private bool _nativeMoveEntered;
    private bool _nativeMoveExited;
    private bool _allowClose;

    internal RecordingFloatingForm(
        RecordingPanelView recordingView,
        System.Drawing.Rectangle initialBounds)
    {
        ArgumentNullException.ThrowIfNull(recordingView);
        if (initialBounds.Width <= 0 || initialBounds.Height <= 0)
        {
            throw new ArgumentOutOfRangeException(
                nameof(initialBounds),
                "Panel 4 Home bounds must have a positive size.");
        }
        ...
        global::Avalonia.Controls.Border floatingSurface = new()
        {
            Child = recordingView,
        };
        floatingSurface.Classes.Add("skill-deck-region");
    ...
    internal event EventHandler<RecordingNativeMovingEventArgs>? NativeMoving;
    ...
    protected override void WndProc(ref Message message)
    {
        ...
        if (message.Msg == WmMoving && message.LParam != nint.Zero)
        {
            ...
            NativeMoving?.Invoke(
                this,
                new RecordingNativeMovingEventArgs(
    ...
    {
        RecordingResizeGrip grip = new(cursor);
...
internal sealed class RecordingResizeGrip : Control
{
    internal RecordingResizeGrip(Cursor cursor)
    {
        SetStyle(
