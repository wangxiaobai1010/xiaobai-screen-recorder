...
internal sealed class DirectorNativeMovingEventArgs(
...
/// content is a second live Avalonia Director view.
/// </summary>
internal sealed class DirectorFloatingForm : Form
{
    ...
    private readonly Dictionary<int, DirectorResizeGrip> _resizeGrips = [];
    private bool _nativeMoveActive;
    private bool _nativeMoveEntered;
    private bool _nativeMoveExited;
    private bool _allowClose;

    internal DirectorFloatingForm(
        DirectorPanelView directorView,
        System.Drawing.Rectangle initialBounds)
    {
        ArgumentNullException.ThrowIfNull(directorView);
        if (initialBounds.Width <= 0 || initialBounds.Height <= 0)
        {
            throw new ArgumentOutOfRangeException(
                nameof(initialBounds),
                "Director Home bounds must have a positive size.");
        }
    ...
    internal event EventHandler<DirectorNativeMovingEventArgs>? NativeMoving;
    ...
    protected override void WndProc(ref Message message)
    {
        ...
        if (message.Msg == WmMoving && message.LParam != nint.Zero)
        {
            ...
            NativeMoving?.Invoke(
                this,
                new DirectorNativeMovingEventArgs(
    ...
    {
        DirectorResizeGrip grip = new(cursor);
...
internal sealed class DirectorResizeGrip : Control
{
    internal DirectorResizeGrip(Cursor cursor)
    {
        SetStyle(
