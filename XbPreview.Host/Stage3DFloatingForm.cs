...
internal sealed class Stage3DNativeMovingEventArgs(
...
/// content is the second live Avalonia Panel 3 view.
/// </summary>
internal sealed class Stage3DFloatingForm : Form
{
    ...
    private readonly Dictionary<int, Stage3DResizeGrip> _resizeGrips = [];
    private bool _nativeMoveActive;
    private bool _nativeMoveEntered;
    private bool _nativeMoveExited;
    private bool _allowClose;

    internal Stage3DFloatingForm(
        Stage3DPanelView stage3DView,
        System.Drawing.Rectangle initialBounds)
    {
        ArgumentNullException.ThrowIfNull(stage3DView);
        if (initialBounds.Width <= 0 || initialBounds.Height <= 0)
        {
            throw new ArgumentOutOfRangeException(
                nameof(initialBounds),
                "Panel 3 Home bounds must have a positive size.");
        }
    ...
    internal event EventHandler<Stage3DNativeMovingEventArgs>? NativeMoving;
    ...
    protected override void WndProc(ref Message message)
    {
        ...
        if (message.Msg == WmMoving && message.LParam != nint.Zero)
        {
            ...
            NativeMoving?.Invoke(
                this,
                new Stage3DNativeMovingEventArgs(
    ...
    {
        Stage3DResizeGrip grip = new(cursor);
...
internal sealed class Stage3DResizeGrip : Control
{
    internal Stage3DResizeGrip(Cursor cursor)
    {
        SetStyle(
