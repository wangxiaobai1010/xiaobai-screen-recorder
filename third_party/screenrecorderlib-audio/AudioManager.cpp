...
HRESULT AudioManager::ConfigureAudioCapture() {
	...
	HRESULT lastResult = S_FALSE;
	const auto observe = [&](const HRESULT result) {
		lastResult = result;
	...
	{
		HRESULT operationResult = S_FALSE;
		if (!m_AudioOutputCapture) {
			m_AudioOutputCapture = make_unique<WASAPICapture>(m_AudioOptions, L"AudioOutputDevice");
			operationResult = m_AudioOutputCapture->Initialize(GetAudioOptions()->GetAudioOutputDevice(), eRender);
			LOG_DEBUG("Created WASAPI capture on %s", m_AudioOutputCapture->GetTag().c_str());
		}
		if (!m_AudioOutputCapture->IsCapturing()) {
			operationResult = StartDeviceCapture(m_AudioOutputCapture.get(), GetAudioOptions()->GetAudioOutputDevice(), eRender);
		}
		observe(operationResult);
	}
	else {
		observe(StopDeviceCapture(m_AudioOutputCapture.get()));
	}
	...
	if (GetAudioOptions()->IsAudioEnabled() && GetAudioOptions()->IsInputDeviceEnabled() && m_IsCaptureEnabled)
	{
		...
		if (!m_AudioInputCapture) {
			...
			operationResult = m_AudioInputCapture->Initialize(GetAudioOptions()->GetAudioInputDevice(), eCapture);
			LOG_DEBUG("Created WASAPI capture on %s", m_AudioInputCapture->GetTag().c_str());
		}
		if (!m_AudioInputCapture->IsCapturing()) {
			operationResult = StartDeviceCapture(m_AudioInputCapture.get(), GetAudioOptions()->GetAudioInputDevice(), eCapture);
		}
		observe(operationResult);
	}
	else {
		observe(StopDeviceCapture(m_AudioInputCapture.get()));
	}
	return FAILED(firstFailure) ? firstFailure : lastResult;
}

HRESULT AudioManager::GetCaptureResult()
{
	EnterCriticalSection(&m_CriticalSection);
	LeaveCriticalSectionOnExit leaveOnExit(&m_CriticalSection);
	if (!m_AudioOptions || !m_IsCaptureEnabled) {
		return S_FALSE;
	}
	if (m_AudioOptions->IsOutputDeviceEnabled()) {
		if (!m_AudioOutputCapture) {
			return E_UNEXPECTED;
		}
		const HRESULT outputResult = m_AudioOutputCapture->GetCaptureResult();
	...
	if (m_AudioOptions->IsInputDeviceEnabled()) {
		if (!m_AudioInputCapture) {
			return E_UNEXPECTED;
		}
		const HRESULT inputResult = m_AudioInputCapture->GetCaptureResult();
